...
# y: Length 44274 vector of category judgments across all subjects and trials. Takes on values of 1, 2, or 3
# d: 540 X 540 matrix of distances between training stimuli, as calculated from our MDS solution
# train: Indices of training stimuli (see above)
# nstim: Number of stimuli in the experiment (540)
# ncat: Number of categories in the experiment (3)
# ntrain: Number of training stimuli in the experiment (180)
# ntrainpercat: Number of training stimuli per category (60)
# nsubj: Number of subjects in the experiment (82)
# nrows: Number of individual judgments (i.e. rows in the dataset; 44274)
# subid: Length 44274 vector of subject id numbers, indicating which subject made each of the judgments in y
# stimid: Length 44274 vector of stimulus numbers, indicating which stimulus is being judged in y
#######################################################

model = run.jags(model = "HierarchicalGCM.R",
                 data = list(
                   'y' = as.numeric(data$catresponse),
                   'd' = distances,
                   'train' = train,
                   'nstim' = length(unique(data$rocknumber)),
                   'ncat' = ncat,
                   'ntrain' = length(train),
                   'ntrainpercat' = length(train)/ncat,
                   'nsubj' = length(unique(data$subid)),
                   'nrows' = nrow(data),
                   'subid' = data$seqid,
                   'stimid' = data$stimid
                 ),
                 monitor = c('chyper','gamma','c','predy'),
                 n.chains = 4,
                 adapt = 500,
                 burnin = 0,
                 sample = 1000,
...
# Create plots of chyper, gamma, and all c parameters. 
# I forego plotting predy here; otherwise I'd have over 44,000 plots!
plot(model, vars = c("chyper","gamma","c"))

# ---- Read model and process predictions ----
# Collapse by rock number and plot predicted vs. observed

# Lines 110-133 dig into the MCMC samples and give back 'propy',
...
# Lines 135-150 read in the actual aggregated rock data
...
plot(pred, obs); abline(0,1); title("Hierarchical GCM")